...
retrouver_donnees <- function(periode_analyse,code_ville,langue) {
  ...
  ipc_mensuel <- readRDS("IPC_mensuel.rds")

  liste_var <- c("ville","code","libelle",
                 var_analyse_m12,var_analyse_m3,var_analyse_m2,var_analyse_m1,var_analyse)
  
  ipc_mensuel_note <- ipc_mensuel %>%
    filter(ville ==  code_ville) %>%
  ...
  pdt_alim_1mois_hausse <- ipc_mensuel_note %>%
  ...
  pdt_alim_1mois_baisse <- ipc_mensuel_note %>%
    filter(nchar(code) == 4 & substr(code,1,2) == "01" | code == "0220" ) %>%
    left_join(lib3_texte, by = c("code" = "code")) %>%
    select(ville,libelle_texte_fr,evol_1mois) %>%
    filter(evol_1mois <0 ) %>%
  ...
  pdt_nonalim_1mois_hausse <- ipc_mensuel_note %>%
  ...
  pdt_nonalim_1mois_baisse <- ipc_mensuel_note %>%
    filter(nchar(code) == 2 & !substr(code, 1, 2) %in% c("01", "02")) %>%
    left_join(lib3_texte, by = c("code" = "code")) %>%
    select(ville,libelle_texte_fr,evol_1mois) %>%
    filter(evol_1mois <0 ) %>%
  ...
  pdt_alim_12mois_hausse <- ipc_mensuel_note %>%
    filter(nchar(code) == 4 & substr(code,1,2) == "01" | code == "0220" ) %>%
    left_join(lib3_texte, by = c("code" = "code")) %>%
  ...
  pdt_alim_12mois_baisse <- ipc_mensuel_note %>%
    filter(nchar(code) == 4 & substr(code,1,2) == "01" | code == "0220" ) %>%
    left_join(lib3_texte, by = c("code" = "code")) %>%
    select(ville,libelle_texte_fr,evol_12mois) %>%
    filter(evol_12mois <0 ) %>%
  ...
  # pdt_nonalim_12mois_hausse <- ipc_mensuel_note %>%
  #   filter(nchar(code) == 2 & !substr(code, 1, 2) %in% c("01", "02")) %>%
  #   left_join(lib3_texte, by = c("code" = "code")) %>%
  #   select(ville,libelle_texte_fr,evol_12mois) %>%
  #   filter(evol_12mois >0 ) %>%
  ...
  #   pull(libelle_final)
  # 
  # pdt_nonalim_12mois_baisse <- ipc_mensuel_note %>%
  #   filter(nchar(code) == 2 & !substr(code, 1, 2) %in% c("01", "02")) %>%
  #   left_join(lib3_texte, by = c("code" = "code")) %>%
  #   select(ville,libelle_texte_fr,evol_12mois) %>%
  #   filter(evol_12mois <0 ) %>%
  ...
  phrase03_texte_alim_1mois_hausse <- concat_fr(pdt_alim_1mois_hausse)
  phrase03_texte_alim_1mois_baisse <- concat_fr(pdt_alim_1mois_baisse)
  phrase04_texte_nonalim_1mois_hausse <- concat_fr(pdt_nonalim_1mois_hausse)
  phrase04_texte_nonalim_1mois_baisse <- concat_fr(pdt_nonalim_1mois_baisse)
  phrase05_texte_alim_12mois_hausse <- concat_fr(pdt_alim_12mois_hausse)
  phrase05_texte_alim_12mois_baisse <- concat_fr(pdt_alim_12mois_baisse)
  # phrase06_texte_nonalim_12mois_hausse <- concat_fr(pdt_nonalim_12mois_hausse)
  # phrase06_texte_nonalim_12mois_baisse <- concat_fr(pdt_nonalim_12mois_baisse)

  # #####   phrase01   #######
  evol_1mois <- ipc_mensuel_note %>%
    filter(code=='000GEN') %>%
    pull(evol_1mois)
  ...
  #####   phrase3   #######
  evol_alim_1mois <- ipc_mensuel_note %>%
    filter(code=='001ALIM') %>%
    pull(evol_1mois)
  ...
  #####   phrase4   #######
  evol_nonalim_1mois <- ipc_mensuel_note %>%
  ...
  #####   phrase5   #######
  evol_alim_12mois <- ipc_mensuel_note %>%
    filter(code=='001ALIM') %>%
    pull(evol_12mois)
  ...
  #####   phrase6   #######
  evol_nonalim_12mois <- ipc_mensuel_note %>%
    filter(code=='001NONALIM') %>%
    pull(evol_12mois)
  ...
  resultat <- list(
    ...
    # texte_alim_1mois_hausse = phrase03_texte_alim_1mois_hausse,
    # texte_alim_1mois_baisse = phrase03_texte_alim_1mois_baisse,
